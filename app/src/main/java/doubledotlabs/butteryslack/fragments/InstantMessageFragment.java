...
public class InstantMessageFragment extends BaseMessageFragment {
    ...
    Action<List<BaseItemAdapter.BaseItem>> loadPage(final String timestamp) {
        return new Action<List<BaseItemAdapter.BaseItem>>() {
            ...
            protected List<BaseItemAdapter.BaseItem> run() throws InterruptedException {
                ...
                JsonObject json = new Gson().fromJson(getButterySlack().session.postGenericSlackCommand(params, "im.history").getReply().getPlainAnswer(), JsonObject.class);
